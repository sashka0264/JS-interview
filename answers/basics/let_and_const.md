...
```
Также переменные через var можно обьявлять с одним и тем же названием можество раз, значение просто перезапишется. А с let/const будет ошибка, потому что название переменной дожно быть единственным и второй раз переменную с тем же названием не выйдет.
