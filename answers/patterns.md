...
### Почему редюсеры - чистые функции:
1. удобно для тестирования, т.к. нет side-эффектов.
2. для производительности редакса. 

### Про Рендер
Если рендерится родительский компонент - у детей тоже будет вызван рендер. Т.к. обьект стейта изменился и все что мы передали детям, идет из нового обьекта. Поэтому рендер срабатывает, а чтобы сделать сравнение входящих обьектов пропсов и нужен PureComponent. Он делает поверхностное сравнение
...
MVC - модель (Redux), представление (React), контроллер (React-redux) 
MVVM - 
MVP -