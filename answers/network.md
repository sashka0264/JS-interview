❗️(1) CORS - механизм, позволяющий делать кросс-доменные запросы с помощью специальных HTTP-заголовков. Он позволяет безопасно делать запросы между разными доменами (например, между domen1.com и domen1.ru/domen2.com). На сервере нужно установить заголовки CORS, которые будут определять, какие домены имеют разрешение на доступ к ресурсам. Без CORS вредоносные сайты могли бы получать доступ к защищенным данным, таким как счета в банке, делая запросы в банк якобы от имени клиентов.
